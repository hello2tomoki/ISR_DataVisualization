...
CREATE PROCEDURE  [ODS_Rpt].[GenerateRChartBinary_Market_Value_By_Rating] (@QY_Date DATE) AS
BEGIN

--DECLARE @QY_Date DATE = '2018-07-31';

DECLARE @ChartInputSource NVARCHAR(MAX) = 
N'select Rating, Weights, [Type]  from [Stg_Rpt].[FI_Per_Market_Value_by_Rating]

Where cast(Weights as float) > 0.001 and

 cast(QY_Date as Date) =
...
png(filename = image_file, width=800, height=400); 
...
d1 <- dataset[dataset$Class == "IG", ]
d2 <- dataset[dataset$Class == "HY", ]
...
d1$text <- paste(round(100 * d1$value, 1), "%", sep = "")
...
d2$text <- paste(round(100 * d2$value, 1), "%", sep = "")
...
gg <- ggplot() + geom_polygon(data=pie1,aes(x,y,fill = group)) +
  geom_polygon(data=pie2,aes(x,y,fill = group)) + 
  # update for font color and size
  geom_text(data=d1, size = 3, color = "#595959",
                  aes(x=1.2*r1*sin(midPercen*2*pi),y=1.3*r1*cos(midPercen*2*pi), 
                      label = paste(key, "\n", text)),
                  alpha = 0.6)+
  # update for font color and size
  geom_text(data=d2, size = 3, color = "#595959",
                  aes(x=c2 + 1.2*r2*sin(midPercen*2*pi),y=1.47*r2*cos(midPercen*2*pi), 
                      label = paste(key, "\n", text)),
                  alpha = 0.6)+
  # update font (High Yield) size and color
  geom_text(size = 3, color = "#595959", fontface = 2, aes(x = c2, y = r2 * 2, label = "High Yield")) + 

CREATE PROCEDURE [Rpt].[GenerateChartBinary_ProvincialDebitIssuanceWaterfall] (@RiskDate DATE) AS
BEGIN

DECLARE @ReportId INT = 0;
DECLARE @ChartTitle VARCHAR(100)= 'Waterfall Provincial Debt Issue';
...
p <- WaterfallChart(dataset$Province, dataset$debtIssue, F) +
  ggtitle("Provincial - Debit Issuance(M$)") +

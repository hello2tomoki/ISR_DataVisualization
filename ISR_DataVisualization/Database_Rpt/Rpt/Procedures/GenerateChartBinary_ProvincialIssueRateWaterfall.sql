CREATE PROCEDURE [Rpt].[GenerateChartBinary_ProvincialIssueRateWaterfall] (@RiskDate DATE) AS
BEGIN

DECLARE @ReportId INT = 0;
DECLARE @ChartTitle VARCHAR(100)= 'Waterfall Provincial Issue Rate';
...
colnames(dataset) <- c("Province", "issueRate")
p <- WaterfallChart(dataset$Province, dataset$issueRate, T) +
  ggtitle("Provincial - BCI % of Issue") +

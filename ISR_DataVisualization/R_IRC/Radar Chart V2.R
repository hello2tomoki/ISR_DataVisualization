#Adapted from https://github.com/region-spotteR/PrepPlot

####====== Primary initial settings =======####

#set working directory (change)
setwd("//bcimcs2/exie$/Documents/Data Visualization")
#upload axis file
grid.data <- read.csv("axis.csv")
#upload csv file
input.data <- read.csv("data.csv")
...
grid <- NULL
grid$axis_path  <- CalculateAxisPath2(grid.data$Axis.Name,grid.min,grid.max+abs(centre.y))

n.vars <- nrow(grid.data)


## 1.2 Calculate the coordinates for the axis labels
grid$axis_label <-funcCircleCoords(0,(grid.max+abs(centre.y))*axis.label.offset, n.vars + 1)
grid$axis_label$text <- c(as.character(grid.data$Axis.Name), "")


## 1.3 Calculate the grid-lines
r<-seq(0,1,0.25) ## Radius of the gridlines
grid$lines <- lapply(1:length(r),function(i) funcCircleCoords(0,r[i]+abs(centre.y), n.vars + 1))
...
for (i in 1:n.vars) {
  min <- grid.data$Min[i]
  max <- grid.data$Max[i]
  if (grid.data$Scale.Direction[i] == "Increasing") {
    qval <- min + r*(max-min)
  }
  if (grid.data$Scale.Direction[i] == "Decreasing") {
    qval <- max - r*(max-min)
  }
...
colnames(qdata) <- c(colnames(qdata)[1:n.vars], colnames(qdata)[1])

grid$lines<-lapply(1:length(grid$lines),function(j) 
  cbind(grid$lines[[j]],values=round(qdata[names(grid$lines[j]),],2)))
...
plot.data <- NULL
for (i in 2:ncol(input.data)) {
  
  min <- grid.data$Min[i-1]
  max <- grid.data$Max[i-1]
  
  if(grid.data[i-1, 2] == "Increasing") {
    plotvar <- (input.data[, i] - min) / (max - min)
  }
  if(grid.data[i-1, 2] == "Decreasing") {
...
input.hover <-  cbind(input.data[,2:ncol(input.data)], input.data[,2])
rownames(input.hover) <- input.data[,1]
colnames(input.hover) <- c(colnames(input.data)[-1], colnames(input.data)[2])
